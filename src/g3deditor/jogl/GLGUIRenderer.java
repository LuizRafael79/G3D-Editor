...
public final class GLGUIRenderer
{
	...
	public final void init(final GL2 gl)
	{
		...
		try
		{
			...
			_fontTexture.enable(gl);
			_fontTexture.setTexParameteri(gl, GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_LINEAR);
			_fontTexture.setTexParameteri(gl, GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_LINEAR);
			
			_compassTexture = TextureIO.newTexture(new File("./data/textures/compass.png"), false);
			_compassTexture.enable(gl);
			_compassTexture.setTexParameteri(gl, GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_LINEAR);
			_compassTexture.setTexParameteri(gl, GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_LINEAR);
		}
		...
		public final void setText(final String text)
		{
		    ...
		    if (text != null) {
		        for (int i = 0; i < text.length(); i++) {
		            ...
		            // mapear para 0–255 (limitar a ASCII/índice da lista)
		            _buffer.put((byte)(c & 0xFF));
		        }
		    }
		    _buffer.flip();
		}
